...
Статус: **применена к production 2026-08-22 с подтверждения владельца.** Фактический прогон,
включая один дефект и его исправление, записан в §0. Ниже — та же процедура, по которой это было
сделано; она остаётся действующей инструкцией для следующей миграции.

---

## 0. Что было выполнено 2026-08-22

| Шаг | Результат |
|---|---|
| Состояние до | схема 2, ingest-таймер уже остановлен с 21.08 18:16 UTC, backup-таймер активен, релиз `…issue_perimeter_cb2bd80f030c` |
| Backup до | `/var/backups/radar-kx/manual-20260822T162033Z-pre-003/radar_kx.dump`, 319 082 605 байт, sha256 `bacb814593e0d1582191a526dec8fbaeafb19c5e66604f66d6de0e4bdc72e380`, `pg_restore --list` прошёл |
| Миграция | применена одной транзакцией, `schema_version` 2 → 3, таблиц в `kx` 28 → 34, вьюх 3 |
| Релиз | `radar_kx_release_20260822_ea37afef6b28`, указатель переключён атомарно |
| `verify --full` | `status: ok`, `errorCount: 0` |
| Backfill провенанса | 25 версий, плюс исправление на 9 (см. ниже) |
| Корпус канона | 36 документов, 827 чанков, 3 229 481 символ, `source_kind = 'canon_import'`, в очереди загрузки 0 |
| `coverage-report` | `status: ok`; периметр 275/275; дым ru 1 039 ≥ 993, en 662 ≥ 633 |
| Backup после | `/var/backups/radar-kx/manual-20260822T162801Z-post-003/radar_kx.dump`, sha256 `8b27dea5c2149e5a217c464512d6616d490dd2b0d1df7936d46ccdedf3aca8b3` |
| Таймеры | оба возвращены в active; первый прогон ingest прошёл чисто за 2,2 с |

### 0.1. Дефект, найденный первым же прогоном backfill

Первый `record-provenance` записал **34 строки вместо 25**. `record_version_provenance` пишет
провенанс **всем** версиям документа, а у трёх документов из двадцати пяти есть более ранние
неполные захваты — legacy-снимки и оборванные сетевые загрузки. Девять строк утверждали, что текст
передан оператором, хотя это были обычные HTTP-загрузки, не дошедшие до конца.

Исправлено в тот же прогон: коррекция теперь несёт `source_kinds`, поставляемый backfill называет
`operator_artifact`, а `data/provenance-correction-2026-08-22.json` дописывает правду для девяти,
ограничившись сетевыми и legacy-ступенями. Ложные строки **остались в истории** — в этом и смысл
append-only: «мы так считали, потом исправили» и есть та запись, которая нужна.

Регресс-тест засевает документ двумя версиями, полученными по-разному, и проверяет, что коррекция
с фильтром оставляет второй версии **отсутствие** провенанса, а не неверный провенанс.

### 0.2. Таймер ingest возвращён, но 2 378 failed сами не пойдут

Отдельная неточность, которую стоит знать: включение таймера ретраев **не возобновляет**.
`claim_tasks` выбирает только `pending` и `retry`; 2 378 строк лежат в статусе `failed` и остаются
там до явного `radar_kx requeue-failed`. Из них 2 289 не исчерпали лимит попыток. Массовый requeue
— отдельное решение: 1 745 из отказов это `robots_denied` на Reddit, и они откажут снова.
