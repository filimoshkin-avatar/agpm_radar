...
class Queue:
    ...
    #: A tab that shows what is in force rather than what awaits a signature.
    #: The wall is a list of pending decisions, and a section that always reads
    #: zero on it teaches the reader to ignore a zero.
...
RETIRED: tuple[RetiredQueue, ...] = (
    RetiredQueue(
        ...
        reason=(
            "Эксперимент закончен, и его ответ уже встроен в конвейер. 220 голосов: ни один метод "
            "не победил в 195 случаях, смысловой в 17, оба в 7, словесный в 1. Связывание этапа 2 "
            "построено ровно на этом выводе — шорт-лист требует обоих методов сразу и отдаёт пару "
            "судье-модели. Восемь оставшихся карточек к выборке с таким перекосом ничего не "
            "добавят."
        ),
        returns_when="понадобится перемерить методы на другом корпусе или на другом эмбеддере",
    ),
    RetiredQueue(
        queue=Queue(
            key="families",
        ...
        reason=(
            "Гейт, который вытеснен и ни разу не "
            "сработал. Семья нужна была, чтобы документы одного "
            "издателя не считались двумя независимыми подтверждениями. Но живой гейт решения 6 "
            "считает различные первоисточники и не упоминает семьи ни разу, а из 6 664 "
            "опубликованных цитат ни одна не имеет "
            "двух источников независимости. Все 155 семей уже "
            "подтверждены. Решение 1 и проход прочтения заменили механизм: четыре издания с одним "
            "прогнозом Gartner различает первоисточник, а не домен."
        ),
        returns_when="независимость снова начнут считать по изданиям, а не по первоисточникам",
    ),
    RetiredQueue(
        queue=Queue(
            key="duplicates",
        ...
        reason=(
            "То же назначение, что у семей, и ещё слабее: пять кластеров, ни одного "
            "подтверждённого, и подтверждение сегодня не меняет ничего ниже по течению — кластеры "
            "читает только сама очередь и один счётчик отчёта."
        ),
        returns_when="дубликаты начнут влиять на счёт подтверждений или на публикацию",
    ),
    RetiredQueue(
        queue=Queue(
            key="ideas",
        ...
        reason=(
            "Слой, который предшествовал тому, что его заменило. Покрытие — 168 утверждений из 13 "
            "876, это 1,2 %. Гейт независимости у него по семьям, то есть по вытесненному "
            "механизму. Таблица оценок пуста, и её форма — одно поле балла — противоречит решению "
            "5: четыре оси, без сводного балла. Наружу не выходит совсем. Сегодня у каждого "
            "утверждения есть предмет, допуск, вид и связи — то, что идея приближала группировкой."
        ),
        returns_when="UC-08 доедет с четырьмя осями решения 5 в схеме",
    ),
    RetiredQueue(
        queue=Queue(
            key="evidence",
...
#: The retired ones, by the same key. Off the wall and out of `decide`, but still
#: loadable - which is what makes retirement reversible and testable.
RETIRED_BY_KEY = {item.queue.key: item.queue for item in RETIRED}
