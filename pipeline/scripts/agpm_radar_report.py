...
from radar_language_quality import foreign_script_fragments, foreign_latin_words, brand_words
...
def needs_russian_summary(value: str) -> bool:
    if not value:
        return True
    return bool(foreign_script_fragments(value) or foreign_latin_words(value, brand_words(value))) or latin_count(value) > cyrillic_count(value) * 2
...
def russian_summary(item: dict[str, Any]) -> str:
    ...
    if "atlassian.com" in item_url and "project-management/ai-agents" in item_url and language:
        return (
            f"Материал Atlassian на {language} языке описывает ИИ-агентов в управлении проектами: автоматизацию задач, "
            "приоритизацию работы, поддержку решений и координацию проектной деятельности. По смыслу это локализованная версия "
            "материала о применении ИИ-агентов в управлении проектами; такие языковые версии нужно проверять на дубли с базовой страницей."
        )
    ...
    if "clawbud" in text:
        return (
            "Материал рассматривает агентную операционную среду для OpenClaw: когда агент получает браузер, каналы, файлы, данные о клиентах и интеграции, "
    ...
    if "monday.com" in text:
        return (
            "monday.com объявила инфраструктуру для ИИ-агентов внутри рабочей платформы: агенты смогут проходить аутентификацию, "
            "обновлять рабочие процессы, организовывать проекты, запускать автоматизации, формировать отчёты и координировать работу команд."
        )
    ...
    if "agent orchestration" in text or "multi-agent workflows" in text:
        return (
            "Материал объясняет оркестрацию ИИ-агентов в компании: чем сложнее многоагентные рабочие процессы, тем важнее контроль согласованности действий, "
    ...
    if "letsdatascience" in url or "fight for orchestration" in title:
        return (
            "Материал фиксирует сдвиг корпоративного интереса от отдельных моделей к средствам управления для агентов: оркестрации, политике доступа, "
            "логированию действий, наблюдаемости и безопасному подключению инструментов и данных."
        )
    if "agentic pmo" in text:
        return (
            "Материал описывает переход проектного офиса к агентной модели: автономные рабочие процессы помогают планировать, назначать задачи, балансировать загрузку, "
            "обнаруживать риски и готовить отчётность с меньшим объёмом ручной координации."
        )
    if "kiplot" in url:
        return (
            "Материал рассматривает сценарии применения ИИ для проектного офиса и портфельного управления: отчётность, портфельная видимость, контроль выполнения и снижение "
            "административной нагрузки. Для AgPM важна связка ИИ-агента с ответственностью и человеческим контролем."
        )
    if "hpe.com" in url:
        return (
            "Статья описывает применение agentic ИИ в проектном управлении: непрерывный мониторинг, динамическое перепланирование, прогнозирование рисков, "
            "ранние предупреждения и рекомендации по перераспределению ресурсов."
        )
    if "gsdcouncil" in url:
        return (
            "Обзор показывает применение agentic ИИ в проектной работе: автоматизированный трекинг, отчётность, приоритизация задач, управление ресурсами, "
            "коммуникации и проактивное обнаружение рисков."
        )
    if "opkey" in url:
        return (
            "Страница продукта описывает PMO AI Agent для сценариев управления ресурсами и качеством: отслеживание работ, трассируемость, взаимодействие со стейкхолдерами "
            "и снятие части рутинной нагрузки с проектного офиса."
        )
    if "dust.tt" in url:
        return (
            "Материал описывает ИИ-агентов для проектного управления: обработку проектных данных, кросс-инструментальную отчётность, сводки встреч, "
            "декомпозицию задач, обнаружение рисков, планирование ресурсов и контроль дедлайнов."
        )
    if "techplustrends" in url:
        return (
            "Материал описывает переход от ручной координации к ИИ-агентам с заданной целью, которые распределяют задачи, прогнозируют риски "
            "и помогают управлять выполнением проектных работ."
        )
    if "wrike" in url:
        return (
            "Материал описывает ИИ-агентов в платформе управления работой: автономные участники команды могут выполнять многошаговые рабочие процессы без постоянного "
            "ручного запроса, а конструктор агентов позволяет настраивать такие сценарии без кода."
        )

    if summary:
        return (
            "Материал показывает связь ИИ-агентов с управленческим или бизнес-контуром; детали следует проверять по первоисточнику и использовать через фильтр AgPM."
        )
...
def general_material_summary(item: dict[str, Any]) -> list[str]:
    ...
    if title:
        for left, right in (("«", "»"), ('"', '"'), ("“", "”")):
            summary = summary.replace(f"{left}{title}{right}", "")
    ...
    if summary and not needs_russian_summary(summary) and "детали следует проверять" not in summary:
        ...
        first = (
            "Материал описывает переход от отдельных ИИ-помощников к агентным рабочим процессам: агенты получают роль в последовательности действий, "
        ...
        first = (
            "Материал относится к близкому периметру: в нём агентность связана с проектным управлением, работой проектного офиса, портфельной видимостью, "
        ...
        first = (
            "Материал рассматривает управляемость ИИ-агентов: политики, права, контроль действий, риски, соответствие требованиям и "
            "корпоративную эксплуатацию агентных сценариев."
        )
    else:
        first = ru_summary

    if "Материал показывает связь ИИ-агентов" in first or "Краткое содержание не извлечено" in first:
        first = (
            "Материал зафиксирован как релевантный сигнал по рынку ИИ-агентов. Его ценность для радара не в самой новости об ИИ, "
    ...
    if "workflow" in text or "business process" in text or "operations" in text:
        ...
        second = (
            "Использовать материал в радаре стоит через фильтр AgPM: отделять реальное делегирование управленческого действия от маркетинговых заявлений об агентности "
...
def general_agpm_analysis(item: dict[str, Any], review: dict[str, Any]) -> list[str]:
    ...
    if perimeter == "near":
        return [
            "Для AgPM это материал близкого периметра: он помогает уточнять, какие проектные функции можно переводить в агентный режим без подмены управленческой ответственности. В центре здесь не «ИИ вообще», а конкретные практики проектного офиса: наблюдение, статус, риск, поручения, координация и портфельная видимость.",
            "Методически такой материал стоит использовать для операционализации AgPM: описывать границы автономии агента, точки человеческого подтверждения, журнал действий и критерии качества агентной рекомендации. Чем ближе сценарий к изменению плана, сроков, ресурсов или эскалации, тем сильнее должен быть контроль со стороны человека.",
        ]
    if "governance" in text or "policy" in text or "permission" in text or "compliance" in text or "risk" in text:
        return [
            "Для AgPM материал усиливает управленческую линию: агентная система должна быть управляемой, наблюдаемой и ограниченной правилами. Права, политики, журналирование, контроль доступа и трассировка действий становятся не технической деталью, а частью управленческой архитектуры.",
