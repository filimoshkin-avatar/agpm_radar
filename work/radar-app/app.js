...
function renderCard(item) {
  ...
  return `<article class="card">
    ...
    <a class="source-link" href="${item.url}" target="_blank" rel="noopener">первоисточник ↗</a>
  </article>`;
}
...
async function loadIssueMaterials(request) {
  ...
  return (await getJson(`/api/materials?${qs(params)}`)).materials;
}

async function loadIssuePayload(issueDate) {
  if (issueCache.has(issueDate)) return issueCache.get(issueDate);
  const response = await fetch(`${API}/api/issue/${issueDate}`);
